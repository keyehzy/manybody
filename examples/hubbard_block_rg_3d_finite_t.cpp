// Finite-temperature Block Renormalization Group (BRG) for the attractive Hubbard model on a 3D
...
CliOptions parse_cli_options(int argc, char** argv) {
  ...
  cxxopts::Options cli("hubbard_block_rg_3d_finite_t",
                       "Finite-T Block RG for attractive 3D Hubbard model (2x2x2 blocks)");
  // clang-format off
  cli.add_options()
...
brg::BrgStepResult brg_step_finite_t(double t, double U, double mu, double T) {
  ...
  // Renormalized parameters from free energies
  ...
  // Hopping renormalization using thermal average of lambda^2
...
double tune_mu_for_eighth_filling_zero_t(double t, double U, bool& window_exists) {
  const auto geometry = brg::block_3d_2x2x2();
...
double tune_mu_for_eighth_filling_finite_t(double t, double U, double T, bool& window_exists) {
  if (brg::use_zero_temperature(T)) {
    return tune_mu_for_eighth_filling_zero_t(t, U, window_exists);
  }
...
int main(int argc, char** argv) {
  ...
  std::cerr << "=== Block RG for 3D Hubbard (2x2x2 blocks, finite T) ===" << std::endl;
  ...
  for (int n = 0; n < opts.iterations; ++n) {
    ...
    if (mode_b) {
      mu = tune_mu_for_eighth_filling_finite_t(t, U, T_iter, window_exists);
    }
