...
int main(int argc, char** argv) {
  ...
  const Expression momentum_state = orderer.normal_order(vector_to_expression(ground_state, basis));

  const Expression real_space_state = orderer.normal_order(transform_expression(
      fourier_transform_operator, momentum_state, index, FourierMode::Inverse));
  const Expression round_trip = orderer.normal_order(transform_expression(
