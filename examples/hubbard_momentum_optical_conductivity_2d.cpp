...
CliOptions parse_cli_options(int argc, char** argv) {
  ...
  cxxopts::Options cli(
      "hubbard_momentum_optical_conductivity_2d",
      "Compute optical conductivity using 2D momentum-space Hubbard model with sparse matrices");
  // clang-format off
  cli.add_options()
...
std::string hamiltonian_cache_path(const CliOptions& opts, const std::string& component,
  ...
  ss << opts.cache_dir << "/hubbard_2d_" << component << "_L=" << opts.size_x << "x" << opts.size_y
...
std::string current_cache_path(const CliOptions& opts, const std::vector<size_t>& K,
  ...
  std::ostringstream ss;
  ss << opts.cache_dir << "/hubbard_2d_current_L=" << opts.size_x << "x" << opts.size_y
     << "_N=" << opts.particles << "_S=" << opts.spin_projection << "_K=" << K[0] << "," << K[1]
     << "_Q=" << Q[0] << "," << Q[1] << "_D=" << opts.direction << ".bin";
  return ss.str();
}
...
int main(int argc, char** argv) {
  ...
  // Momentum transfer Q
  const std::vector<size_t> Q = {opts.qx, opts.qy};
...
#pragma omp critical
  ...
  // For 2D, use area instead of volume
  const double area = static_cast<double>(opts.size_x * opts.size_y);
  const OpticalConductivityResult result =
      compute_optical_conductivity(correlator, opts.dt, opts.beta, area);
