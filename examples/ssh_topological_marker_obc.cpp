/// SSH Topological Marker Example - Open Boundary Conditions
///
/// This example computes the 1D topological marker for the SSH model
/// using open boundary conditions (OBC). The marker correctly identifies
...
CliOptions parse_cli_options(int argc, char** argv) {
  ...
  cxxopts::Options cli("ssh_topological_marker_obc",
                       "SSH topological marker with open boundary conditions");
  // clang-format off
  cli.add_options()
...
void test_ssh_obc(double t1, double t2, size_t num_cells, size_t kpm_order) {
  std::cout << "=== SSH Model (OBC) ===\n";
  std::cout << "t1 = " << t1 << ", t2 = " << t2 << ", L = " << num_cells << "\n";
  std::cout << "Expected winding number: " << (t2 > t1 ? 1 : 0) << "\n\n";

  // Build model with OBC
...
int main(int argc, char** argv) {
  ...
  std::cout << "========================================\n";
  std::cout << "SSH Topological Marker - OBC\n";
  std::cout << "========================================\n\n";

  test_ssh_obc(opts.t1, opts.t2, opts.num_cells, opts.kpm_order);
  std::cout << "----------------------------------------\n\n";
  test_ssh_obc(opts.t2, opts.t1, opts.num_cells, opts.kpm_order);
