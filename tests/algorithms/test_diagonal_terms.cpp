...
TEST_CASE("diagonal_children_assigns_off_diagonal_to_parents") {
  const auto c0 = FermionOperator::creation(FermionOperator::Spin::Up, 0);
  const auto c1 = FermionOperator::creation(FermionOperator::Spin::Up, 1);
  const auto a0 = FermionOperator::annihilation(FermionOperator::Spin::Up, 0);
  const auto a1 = FermionOperator::annihilation(FermionOperator::Spin::Up, 1);
  ...
  FermionExpression expr({diag_one, diag_two, off_diag});
...
TEST_CASE("diagonal_children_keeps_empty_entries_for_diagonals") {
  FermionMonomial diag_term =
      density_density(FermionOperator::Spin::Up, 0, FermionOperator::Spin::Down, 1);
  FermionMonomial off_diag = one_body(FermionOperator::Spin::Up, 0, FermionOperator::Spin::Down, 2);

  FermionExpression expr({diag_term, off_diag});
  const auto result = group_diagonal_children(expr);
...
TEST_CASE("diagonal_children_matches_operator_substrings") {
  const auto c0 = FermionOperator::creation(FermionOperator::Spin::Up, 0);
  const auto c1 = FermionOperator::creation(FermionOperator::Spin::Up, 1);
  const auto c2 = FermionOperator::creation(FermionOperator::Spin::Up, 2);
  const auto c3 = FermionOperator::creation(FermionOperator::Spin::Up, 3);
  const auto a0 = FermionOperator::annihilation(FermionOperator::Spin::Up, 0);
  const auto a1 = FermionOperator::annihilation(FermionOperator::Spin::Up, 1);
  const auto a2 = FermionOperator::annihilation(FermionOperator::Spin::Up, 2);
  const auto a3 = FermionOperator::annihilation(FermionOperator::Spin::Up, 3);
  ...
  FermionExpression expr({diag, off_prefix, off_suffix, off_none});
  const auto result = group_diagonal_children(expr);
...
TEST_CASE("find_matching_terms_uses_term_containment") {
  ...
  FermionExpression expr({exact, contains_prefix, contains_suffix, contains_middle, none});
...
TEST_CASE("find_matching_terms_handles_single_operator") {
  ...
  FermionExpression expr({exact, embedded, other});
  const auto matches = find_matching_terms(reference, expr);
...
TEST_CASE("find_matching_terms_allows_out_of_order_matching") {
  ...
  FermionExpression expr({contiguous_match, non_contiguous, missing_operator});
  const auto matches = find_matching_terms(reference, expr);
...
TEST_CASE("find_matching_terms_empty_reference_returns_empty") {
  ...
  FermionExpression expr({density_term});
