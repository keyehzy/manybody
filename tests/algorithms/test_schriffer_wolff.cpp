...
TEST_CASE("schriffer_wolff_zero_kinetic_returns_zero_generator") {
  ...
  FermionBasis basis = FermionBasis::with_fixed_particle_number(lattice_size, particles);

  FermionExpression generator =
      schriffer_wolff(hubbard.kinetic(), hubbard.interaction(), basis, 10);
...
TEST_CASE("schriffer_wolff_generator_is_antihermitian") {
  ...
  FermionExpression generator =
      schriffer_wolff(hubbard.kinetic(), hubbard.interaction(), basis, 50);
  arma::cx_mat A = compute_matrix_elements<arma::cx_mat>(basis, generator);
