#include <catch2/catch.hpp>

#include "algebra/fermion/expression.h"

TEST_CASE("canonicalize_zero_coefficient_returns_empty") {
  FermionMonomial::container_type ops{};
  FermionExpression result = canonicalize(FermionMonomial::complex_type(0.0f, 0.0f), ops);
  CHECK((result.size()) == (0u));
}

TEST_CASE("canonicalize_single_operator_is_identity") {
  FermionOperator op = FermionOperator::creation(FermionOperator::Spin::Up, 1);
  FermionMonomial term(op);
  FermionExpression result = canonicalize(term);

  FermionExpression::container_type ops{op};
  auto it = result.terms().find(ops);
  CHECK(it != result.terms().end());
  CHECK((it->second) == (FermionExpression::complex_type(1.0f, 0.0f)));
}

TEST_CASE("canonicalize_commuting_swap_introduces_phase") {
  FermionOperator a = FermionOperator::creation(FermionOperator::Spin::Up, 1);
  FermionOperator b = FermionOperator::creation(FermionOperator::Spin::Up, 2);
  FermionMonomial term(FermionMonomial::complex_type(1.0f, 0.0f), {b, a});
  FermionExpression result = canonicalize(term);

  FermionExpression::container_type ordered{a, b};
  auto it = result.terms().find(ordered);
  CHECK(it != result.terms().end());
  CHECK((it->second) == (FermionExpression::complex_type(-1.0f, 0.0f)));
  CHECK((result.size()) == (1u));
}

TEST_CASE("canonicalize_non_commuting_pair_contracts") {
  FermionOperator create = FermionOperator::creation(FermionOperator::Spin::Down, 3);
  FermionOperator annihilate = FermionOperator::annihilation(FermionOperator::Spin::Down, 3);
  FermionMonomial term(FermionMonomial::complex_type(1.0f, 0.0f), {annihilate, create});
  FermionExpression result = canonicalize(term);

  FermionExpression::container_type empty{};
  auto it_empty = result.terms().find(empty);
  CHECK(it_empty != result.terms().end());
  CHECK((it_empty->second) == (FermionExpression::complex_type(1.0f, 0.0f)));

  FermionExpression::container_type ordered{create, annihilate};
  auto it_ordered = result.terms().find(ordered);
  CHECK(it_ordered != result.terms().end());
  CHECK((it_ordered->second) == (FermionExpression::complex_type(-1.0f, 0.0f)));
  CHECK((result.size()) == (2u));
}

TEST_CASE("canonicalize_consecutive_duplicates_vanish") {
  FermionOperator a = FermionOperator::creation(FermionOperator::Spin::Up, 4);
  FermionMonomial term(FermionMonomial::complex_type(1.0f, 0.0f), {a, a});
  FermionExpression result = canonicalize(term);
  CHECK((result.size()) == (0u));
}
