...
constexpr uint64_t compute_basis_size(uint64_t orbitals, uint64_t particles) noexcept {
  if constexpr (AlgebraTraits<S>::pauli_exclusion) {
    return choose(2 * orbitals, particles);
  } else {
    uint64_t modes = 2 * orbitals;
...
constexpr uint64_t compute_basis_size_spin(uint64_t orbitals, uint64_t up, uint64_t down) noexcept {
  if constexpr (AlgebraTraits<S>::pauli_exclusion) {
    ...
    auto boson_choose = [](uint64_t modes, uint64_t p) -> uint64_t {
      if (modes == 0) return (p == 0) ? 1 : 0;
      return choose(modes + p - 1, p);
    };
    return boson_choose(orbitals, up) * boson_choose(orbitals, down);
  }
}
...
BasisImpl<MonomialType> BasisImpl<MonomialType>::with_fixed_particle_number(size_t orbitals,
                                                                            size_t particles) {
  BasisImpl basis;
  basis.orbitals = orbitals;
  basis.particles = particles;
  assert(orbitals <= operator_type::max_index());
  if constexpr (AlgebraTraits<statistics>::pauli_exclusion) {
    assert(particles <= 2 * orbitals);
  } else {
    assert(particles <= key_type::max_size());
  }

  size_t basis_size = compute_basis_size<statistics>(orbitals, particles);
...
template <typename MonomialType>
BasisImpl<MonomialType> BasisImpl<MonomialType>::with_fixed_particle_number_and_spin(
    size_t orbitals, size_t particles, int spin_projection) {
  BasisImpl basis;
  basis.orbitals = orbitals;
  basis.particles = particles;
  assert(orbitals <= operator_type::max_index());
  ...
  size_t basis_size = compute_basis_size_spin<statistics>(orbitals, up, down);
  std::vector<key_type> acc;
  acc.reserve(basis_size);
...
template <typename MonomialType>
BasisImpl<MonomialType> BasisImpl<MonomialType>::with_fixed_particle_number_spin_momentum(
    size_t orbitals, size_t particles, int spin_projection, const Index& index,
  ...
  size_t max_size = compute_basis_size_spin<statistics>(orbitals, up, down);
...
template <typename MonomialType>
BasisImpl<MonomialType> BasisImpl<MonomialType>::with_all_particle_number(size_t orbitals,
                                                                          size_t particles) {
  BasisImpl basis;
  basis.orbitals = orbitals;
  basis.particles = particles;
  ...
  for (size_t i = 1; i <= particles; ++i) {
    basis_size += compute_basis_size<statistics>(orbitals, i);
  }
  std::vector<key_type> acc;
  acc.reserve(basis_size);
...
template <typename MonomialType>
void BasisImpl<MonomialType>::generate_all_combinations(key_type current, size_t first_orbital,
                                                        std::vector<key_type>& acc) const {
  constexpr bool strict = AlgebraTraits<statistics>::pauli_exclusion;
  ...
  for (int spin_index = 0; spin_index < 2; ++spin_index) {
    for (size_t i = first_orbital; i < orbitals; i++) {
      typename operator_type::Spin spin = static_cast<typename operator_type::Spin>(spin_index);
      bool should_iterate =
          current.empty() ||
          (current.back().value() < i ||
           (current.back().value() == i &&
            (strict ? spin > current.back().spin() : spin >= current.back().spin())));
      if (should_iterate) {
        current.push_back(operator_type::creation(spin, i));
...
template <typename MonomialType>
void BasisImpl<MonomialType>::generate_restrict_combinations(key_type current, size_t first_orbital,
                                                             std::vector<key_type>& acc) const {
  constexpr bool strict = AlgebraTraits<statistics>::pauli_exclusion;
...
template <typename MonomialType>
void BasisImpl<MonomialType>::generate_restrict_combinations_with_spin(
  ...
  for (int spin_index = 0; spin_index < 2; ++spin_index) {
    for (size_t i = first_orbital; i < orbitals; i++) {
      typename operator_type::Spin spin = static_cast<typename operator_type::Spin>(spin_index);
      if ((spin == operator_type::Spin::Up && up_left == 0) ||
          (spin == operator_type::Spin::Down && down_left == 0)) {
        continue;
      }
      bool should_iterate =
          current.empty() ||
      ...
      if (should_iterate) {
        ...
        generate_restrict_combinations_with_spin(
            current, i, up_left - (spin == operator_type::Spin::Up ? 1 : 0),
            down_left - (spin == operator_type::Spin::Down ? 1 : 0), acc);
        current.pop_back();
      }
    }
  }
...
template <typename MonomialType>
void BasisImpl<MonomialType>::generate_restrict_combinations_with_spin_momentum(
    key_type& current, size_t first_orbital, size_t up_left, size_t down_left,
    ...
    for (size_t i = first_orbital; i < orbitals; ++i) {
      ...
      if (should_iterate) {
        ...
        generate_restrict_combinations_with_spin_momentum(
            current, i, up_left - (spin == operator_type::Spin::Up ? 1 : 0),
            down_left - (spin == operator_type::Spin::Down ? 1 : 0), current_momentum,
            orbital_coords, index, target_momentum, acc);
